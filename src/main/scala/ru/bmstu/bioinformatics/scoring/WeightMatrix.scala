...
object WeightMatrix {
  ...
  private lazy val nucleotideMatrix = fromFile(Source.fromResource("nucleotide.mtx"))
  private lazy val proteinMatrix = fromFile(Source.fromResource("protein.mtx"))
   ...
   */
  def fromFile(fileInput: BufferedSource): KeyMatrix = {
    val builder = mutable.HashMap[(Char, Char), Int]()
    val names :: body = fileInput
      .getLines()
      .map(_.trim)
      .filter(!_.startsWith("#"))
      .toList
